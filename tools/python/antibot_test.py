...
def _pause_between_requests() -> None:
    ...
    if PAUSE_BETWEEN_REQUESTS_SEC <= 0:
        return
    sec = PAUSE_BETWEEN_REQUESTS_SEC + random.uniform(
        -PAUSE_DEVIATION_SEC, PAUSE_DEVIATION_SEC
    )
    sec = max(
        PAUSE_BETWEEN_REQUESTS_SEC_MIN, sec
    )  # 60/25 = 2.4/min upper bound; 30±4 gives ~2/min
    time.sleep(sec)
...
def _cookies_jar() -> dict[str, str]:
    ...
    return {str(c["name"]): str(c["value"]) for c in INVENT_COOKIES}
...
def _ja3_hash_from_result(data: dict) -> str:
    ...
    return (
        headers.get("x-fp-ja3-hash")
        or (fp.get("proxy_headers") or {}).get("X-FP-JA3-HASH")
        or "—"
    )
...
def _challenge_state_line(data: dict) -> str:
    ...
    if cd.get("stored_ua"):
        ua_short = (
            cd["stored_ua"][:60] + "…" if len(cd["stored_ua"]) > 60 else cd["stored_ua"]
        )
    ...
    for i, profile in enumerate(profiles):
        ...
        try:
            response = session.get(
                url, impersonate=profile, headers=headers  # type: ignore[arg-type]
            )
            data = response.json()
            status = "PASS" if data["classification"] == "browser" else "FAIL"
            _ja3_hash_from_result(data)
            ...
            print(
                f"[{status}] {profile:20s} -> {data['classification']} "
            ...
            if cd.get("in_store") and cd.get("stored_ua"):
                ...
                if (
                    current_ua
                    and cd["stored_ua"] != current_ua
                    and data["classification"] == "browser"
                ):
