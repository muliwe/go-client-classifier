...
**Current defaults (v1.5.1):** Edge thresholds were calibrated on an extended cohort; **high-request-rate** and **mean-above-median** add **2** points each, **low-inter-arrival-median** and **high-inter-arrival-variance** add **1** point each (weak signals). You can override in `config/scoring.json`.
