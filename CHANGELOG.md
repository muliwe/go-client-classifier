...
- **Signals table cell styling**: Browser and bot columns (counts and percentages) visualize **signal contrast** by intensity: **strong** — browser ≥75% or bot ≥95% → maximum brightness (original green/red); **weak** — ≥25% that side and outside 40–60% → medium (darker green/red); **indistinguishable** — 40–60% → neutral (gray); **not observed** — 0 cases → dark gray. The total column is dark gray when total is 0. Aligns with METHODOLOGY Appendix N (strong / weak / indistinguishable / not observed). See [METHODOLOGY Appendix N](docs/METHODOLOGY.md#appendix-n-dashboard-functionality).
...
- **parseJA3Counts**: New helper parses JA3 format (Version,Ciphers,Extensions,EllipticCurves,PointFormats) and returns cipher IDs, extension IDs, and supported group IDs. Used only when building TLS fingerprint from proxy.
- **has_session_ticket and has_early_data from JA3**: When X-FP-JA3 is present, extension IDs (field 3) are checked for 35 (session_ticket) and 42 (early_data). `has_session_ticket` and `has_early_data` are set so that session-ticket scoring applies for proxied TLS when the client sent those extensions.
...
- **Tests**: `TestCollect_TrustedProxy_JA3FillsCountsAndGroups` verifies counts, OfferedCipherSuites/SupportedGroups names, HasSessionTicket when JA3 has ext 35, and score_breakdown. `TestCollect_TrustedProxy_JA3ExtensionPresence` verifies has_session_ticket/has_early_data from JA3 extension list. `TestCollect_TrustedProxy_ReusesNginxHeaders` checks SupportedVersions. `TestCollect_TrustedProxy_TLSSignalsForScoring` verifies modern-tls and ssl-greased when TLS is from proxy.
...
- **Config**: New top-level `behavioral_edges` in scoring JSON with `request_rate_per_min_above` (2.0), `inter_arrival_median_sec_below` (4.0), `inter_arrival_std_per_mean_above` (1.35), `inter_arrival_mean_median_ratio_above` (1.2). Four `bot_scores` keys: `high-request-rate`, `low-inter-arrival-median`, `high-inter-arrival-variance`, `mean-above-median` — **2 points each** (strong signal). See [config/README.md](config/README.md#behavioural-metrics-optional-appendix-m).
...
- **request_log_stats_by_class.py**: Script outputs **behavioural-edge** statistics: `count_by_signals` (0–4), **count_per_signal** (req_per_min, gap_median, gap_std_mean, gap_mean_median), **bot recall** and **browser FP rate**. Edge defaults match config (2.0, 4.0, 1.35, 1.2). **Cohort labelling**: BOT/BROWSER in the script come from log classification (fingerprint/UA only, before behavioural scoring), so some labels are pre-behavioural; see Appendix M.
- **METHODOLOGY**: Appendix M updated with extended-cohort example run (948 records: recall 34.99%, browser FP 31.02%; count_by_signals and count_per_signal). Cohort labelling note: recall/FP are with respect to pre-behavioural (fingerprint-only) labels.
- **Tests**: Unit tests for ApplyBehavioralSignals (nil metrics, nil botScores, high-request-rate, low-inter-arrival-median, high-variance, mean-above-median, single-request no inter-arrival). Config test updated for 34 bot_scores keys. Behavioural bot_scores expect **2** points each.
